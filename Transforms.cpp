...
bool TransformIntegerConstants(llvm::BasicBlock &BB) {
    ...
    for (auto I = BB.begin(); I != BB.end(); ++I) {
        const auto instruction = llvm::dyn_cast<llvm::ICmpInst>(I);
        ...
        const auto x = instruction->getOperand(0);
        const auto y = instruction->getOperand(1);
        ...
        if (value_replace->getSExtValue() == 0) {
            instruction->setOperand(value_replace_index, zero_expr);
        } else {
            instruction->setOperand(value_replace_index, GetObfuscatedValue(builder, zero_expr, value_replace));
        }
        changed = true;
    }
